...
- `auto-pull.sh` — old cron-driven auto-deploy. **If the mac mini still has
  a crontab entry pointing at the repo-root `auto-pull.sh`, remove it** —
  the script no longer exists at that path.
